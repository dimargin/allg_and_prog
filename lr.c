...
#define A 3 // Количество строк в матрице
#define K 4 // Количество столбцов в матрице

int lr7(void) 
{
    int arr[A][K]; // Объявление исходного двумерного массива
    ...
    if (maxrow != minrow) 
    {
        for (int j = 0; j < K; j++) // Перебор всех элементов строк
        {
    ...
    {
        for (int j = 0; j < K; j++) // Перебор всех элементов строки
        {
